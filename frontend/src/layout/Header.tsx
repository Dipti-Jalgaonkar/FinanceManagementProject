        ...
        <div className="sticky top-0 z-50 bg-white text-gray-900 shadow-[0_1px_0_0_rgba(17,24,39,0.06)] dark:bg-[#0b0b0c] dark:text-white">
            <header className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="flex h-16 items-center justify-between gap-3">
                    <Link
                        to="/"
                        className="shrink-0 rounded-lg text-[18px] font-semibold tracking-tight hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-300 dark:focus-visible:ring-white/15"
                    >
                        FinanceOS
                    </Link>
                    ...
                    <nav className="hidden md:flex md:items-center md:gap-1">
                        {NAV_ITEMS.map((item) => (
                            <Link
                                key={item.label}
                                to={item.to}
                                className="rounded-xl px-3 py-2 text-sm font-medium text-gray-600 transition-colors hover:text-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-300 dark:text-neutral-300 dark:hover:text-white dark:focus-visible:ring-white/15"
                            >
                                {item.label}
                            </Link>
                        ))}
                        ...
                        <nav className="flex flex-col">
                            {NAV_ITEMS.map((item) => (
                                <Link
                                    key={item.label}
                                    to={item.to}
                                    className="rounded-xl px-3 py-2 text-sm font-medium text-gray-800 hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-300 dark:text-neutral-200 dark:hover:bg-white/5 dark:focus-visible:ring-white/15"
                                >
                                    {item.label}
                                </Link>
                            ))}
                            ...
                            <Link
                                to={CTA.to}
                                className="mt-2 inline-flex items-center justify-center rounded-2xl bg-gray-900 px-4 py-2.5 text-sm font-medium text-white shadow-sm transition hover:opacity-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-300 dark:bg-white dark:text-black dark:focus-visible:ring-white/15"
                            >
                                {CTA.label}
                            </Link>
                        </nav>
